...
import { getAccountByID, getActiveAccount, getAllAccounts } from '../accounts';
import { SerializedUIAccount } from '../accounts/Account';
...
export const STORAGE_ACTIVE_ACCOUNT = 'active_account';
...
export class Keyring {
	...
	public async createVault(password: string, importedEntropy?: string) {
		// todo: no-op
		// replaced by `createMnemonicAccountSource`
	}
	...
	deriveNextAccount() {
		console.log('deriveNextAccount called');
		return { address: '0x3', derivationPath: '' };
	}

	importLedgerAccounts(ledgerAccounts: any[]) {
		console.log('importLedgerAccounts called with:', ledgerAccounts);
	}
	...
	async changeActiveAccount(address: string) {
	...
	private storeActiveAccount(address: string) {
		console.log('store active account called');
		return setToLocalStorage(STORAGE_ACTIVE_ACCOUNT, address);
	}

	exportAccountKeypair(address: string, password: string) {
		console.log('exportAccountKeypair called with:', address, password);
	}

	importAccountKeypair(keypair: any, password: string) {
		console.log('importAccountKeypair called with:', keypair, password);
	}
	...
	storeQredoConnection(
		...
		console.log('storeQredoConnection called with:', qredoID, refreshToken, password, newAccounts);
	}
	...
	getQredoRefreshToken(qredoID: string) {
		console.log('getQredoRefreshToken called with:', qredoID);
		return '';
	}

	async handleUiMessage(msg: Message, uiConnection: UiConnection) {
		const { id, payload } = msg;
		// console.log('handleUiMessage called with:', msg, uiConnection);
		// console.log('JSON stringified', JSON.stringify(msg));
		this.postponeLock();
		...
		if (isKeyringPayload(payload, 'create') && payload.args !== undefined) {
			...
			// const account = await uiConnection.createMnemonicAccountSource(password, importedEntropy!);
			// console.log(account);

			// if (account) {
			// 	let acc: SerializedUIAccount = account;
		...
		if (isKeyringPayload(payload, 'unlock') && payload.args) {
		...
		if (isKeyringPayload(payload, 'clear')) {
			await this.clearVault();
			uiConnection.send(createMessage({ type: 'done' }, id));
		}
		if (isKeyringPayload(payload, 'appStatusUpdate')) {
